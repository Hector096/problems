function shorterestTime(n,m,speeds) {
    ...
    let walking = n * speeds[3]
    let elevator =  Math.abs(n-m) * speeds[0] + (speeds[1] + speeds[2] + (speeds[0] * n) + speeds[1])
    
    
    if(speeds[0]*2 > speeds[3] && n>m){
      let time = (speeds[3] * (n-m)) + ((speeds[1] + speeds[2] + (speeds[0] * m) + speeds[1]))
      if(time < walking){
        walking = time
      } else if(time< elevator){
        elevator = time
      }
      else{
        return time
    ...
    return walking > elevator ? elevator : walking
...
// assert.strictEqual(shorterestTime(4,5,[1,2,3,10]),12);
//     assert.strictEqual(shorterestTime(0,5,[1,2,3,10]),0);
//     assert.strictEqual(shorterestTime(4,4,[1,2,3,10]),11);
//     assert.strictEqual(shorterestTime(1,1,[1,2,3,10]),8);
//     assert.strictEqual(shorterestTime(1,1,[2,3,4,10]),10);
//     assert.strictEqual(shorterestTime(4,3,[1,2,3,10]),12);
//     assert.strictEqual(shorterestTime(4,3,[2,3,4,5]),20);
//     assert.strictEqual(shorterestTime(7,6,[3,1,1,4]),25);